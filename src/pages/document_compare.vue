<template>
  <div>
    ...
    <!-- ================= -->
    <p class="table_caption">Таблица документов для сравнения</p>
    <div class="OverflowY_scroll">
      <table class="table_blur tbody-compare-table">
        <thead>
          <tr>
            <th>ID документа</th>
            <th>Название</th>
        ...
        <transition-group name="Animation_ViewItems" tag="tbody">
          <tr v-if="IsLoadingItemsCompare" :key="1">
            <td colspan="3">
              <loadingsmall
                :IsLoading="IsLoadingItemsCompare"
                :center="true"
                style="width:100%"
              ></loadingsmall>
            </td>
          ...
          <tr
            v-else-if="
              GetCompareDocLost.length <= 0 && IsLoadingItemsCompare === false
            "
            :key="2"
          >
          ...
          <tr
            v-else
            v-for="(value, index) in GetCompareDocLost"
            :key="index + 2"
          >
            <td>{{ value.doc["id"] }}</td>
            <td>
              <img
                v-show="
                  value.doc.consultant_link !== null &&
                    value.doc.consultant_link !== ''
                "
                src="../assets/img/KSPlust-icon.png"
                alt
              />
              <router-link :to="'/docview/' + value.doc['id']">{{
                value.doc["title"]
              }}</router-link>
            </td>
            <td>
              <button
                ...
                @click="DeleteItemFromTableCompare(index)"
              >
                Удалить
              </button>
            </td>
    ...
    <button :disabled="CompareDocList.length <= 1" class="btn btn-success" @click="GetCompare()">
      Сравнить документы
    </button>
    <!-- ================= -->
              ...
              <button
                :disabled="
                  (value.consultant_link !== null &&
                    ...
                    FindIndexCompareList(value['id'])
                "
                class="btn btn-success"
                @click="AddDocInTableCompare(value, index)"
              >
...
export default {
  ...
  methods: {
    GetIdCompareDoc(index) {
      if (this.CompareDocList.length > 1) {
        return this.CompareDocList[index].doc.id;
      } else {
        return -1;
      }
    },
    ...
    FindIndexCompareList(id) {
      if (this.CompareDocList.length > 0) {
        for (const item of this.CompareDocList) {
          if (item.doc.id === id) {
            return true;
          }
        }
    ...
    GetCompare() {
      if (this.CompareDocList.length > 1) {
        this.$router.push(`/console/document-view-compare/?sid=${this.GetIdCompareDoc(0)}&cid=${this.GetIdCompareDoc(1)}&page=1`);
      }
    },
    DeleteItemFromTableCompare(index) {
      if (this.CompareDocList.length > 0 && index >= 0) {
        this.CompareDocList.splice(index, 1);
      }
    },
    AddDocInTableCompare(value, index) {
      if (this.CompareDocList.length <= 1) {
        this.IsLoadingItemsCompare = true;
        this.CompareDocList.push({ doc: value });
        this.IsLoadingItemsCompare = false;
      } else {
        this.ViewNotification(
          "Внимание",
          "Ошибка! Нельзя больше 2 документов добавить в таблицу сравнения",
          "error"
        );
      }
    },
    ...
    async GetViewAllDoc(_page) {
      ...
      if (this.PageNum > 1) this.$router.push("/console/document-compare/1");
      this.PageNum = 1;
      this.IsSearch = false;
      try {
  ...
  computed: {
    GetCompareDocLost() {
      return this.CompareDocList;
    },
    GetDocumentList() {
      return this.$store.getters.GetDocCompareList;
    },
    GetDocCategory() {
...
.tbody-compare-table td:first-child {
  width: 150px;
}

.tbody-compare-table td:last-child {
  width: 150px;
}
